...
export const CompanyFastInfoTab: React.FC<CompanyFastInfoTabProps> = ({
  ...
  // 分類清單配置
  const categories: { id: 'all' | AnnouncementCategory; label: string }[] = useMemo(() => [
    { id: 'all', label: '全部公告' },
  ...
  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = { all: infoData?.announcements?.length || 0 };
  ...
  const formattedUpdateTime = useMemo(() => {
    if (!infoData?.lastUpdated) return '';
    const date = new Date(infoData.lastUpdated);
  ...
  if (loading && !infoData) {
    return (
      <div className="space-y-6 animate-pulse select-none">
        {/* 控制列骨架 */}
        <div className="p-4 rounded-xl bg-pro-bg border border-pro-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-32 h-6 rounded bg-pro-panel" />
            <div className="w-24 h-6 rounded bg-pro-panel" />
          </div>
          <div className="w-24 h-8 rounded bg-pro-panel" />
        </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in text-pro-text select-text">
      {/* 頂部控制與狀態資訊列 (Control Bar) */}
      <div className="p-4 rounded-xl bg-pro-bg border border-pro-border flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/25 text-emerald-300 text-xs font-medium">
            <Radio className={`w-3.5 h-3.5 text-emerald-400 ${infoData?.isLive ? 'animate-pulse' : ''}`} />
            <span>{infoData?.dataSourceDesc || '公開資訊觀測站 (MOPS) & 官方日程'}</span>
          </div>

          {/* 快取與同步狀態 */}
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-pro-panel border border-pro-border text-pro-muted text-xs font-mono">
            <Clock className="w-3 h-3 text-pro-muted" />
            <span>{infoData?.isLive ? '即時同步中' : '15分鐘快照快取'}</span>
            {formattedUpdateTime && (
              <span className="text-[11px] opacity-80">· {formattedUpdateTime}</span>
            )}
          </div>

          {/* 市場別標籤 */}
          <span className="px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-white/5 border border-white/10 text-pro-textSec">
            {infoData?.market === 'TW' ? '🇹🇼 台股證交所/櫃買' : infoData?.market === 'US' ? '🇺🇸 美股 SEC 規範' : '全球市場'}
          </span>
        </div>
              ...
              return (
                ...
                >
                  ...
                  {event.highlights && event.highlights.length > 0 && (
                    <div className="mt-2.5 pt-2 border-t border-pro-border/50 text-[11px] text-pro-textSec line-clamp-2 leading-relaxed">
                      {event.highlights[0]}
                    </div>
                  )}
                </div>
      ...
      <div className="p-5 rounded-xl bg-pro-bg border border-pro-border space-y-4">
        ...
        {/* 分類篩選按鈕列 (Filter Pills) */}
        <div className="flex flex-wrap items-center gap-1.5 pb-2 border-b border-pro-border">
          {categories.map((cat) => {
      ...
      {/* 權威財經即時新聞 (Financial News Feed) */}
      <div className="p-5 rounded-xl bg-pro-bg border border-pro-border space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Newspaper className="w-4 h-4 text-emerald-400" />
            <h3 className="text-sm font-bold text-white">權威財經即時新聞 (Financial News Feed)</h3>
          </div>
          <span className="text-xs text-pro-muted">工商時報 · 經濟日報 · 鉅亨網 · Yahoo財經</span>
        </div>
            ...
            {infoData.news.map((item) => (
              ...
              >
                <div>
                  ...
                  <h4 className="text-xs font-bold text-white mt-2 line-clamp-2 leading-snug hover:text-emerald-300 transition-colors">
                    {item.title}
                  </h4>

                  <p className="text-[11px] text-pro-muted mt-1.5 line-clamp-2 leading-relaxed">
                    {item.summary}
                  </p>
                </div>
