...
} from './proposed_mopsService';
import {
  CompanyFastInfoState,
} from './proposed_companyInfo';

// ============================================================================
// LocalStorage Mock 環境隔離設定
// ============================================================================
const storageMock = (() => {
...
describe('Explorer M1 Data Verification Suite', () => {
  beforeEach(() => {
    localStorage.clear();
    clearCompanyFastInfoCache();
    vi.restoreAllMocks();
  ...
  describe('民國年日期轉換 (parseRocDate)', () => {
    it('應將 7 碼純數字民國日期正確轉換為西元 YYYY-MM-DD', () => {
  ...
  describe('法定財報與營收行事曆生成 (generateStatutoryCalendar)', () => {
    ...
    it('以 2026-11-20 為基準日時，下一次法定財報應跨年推進至 2027/03/31 (Q4 年報)', () => {
    ...
    it('當基準日超過每月 10 日時，營收申報截止應自動推進至次月 10 日', () => {
  ...
  describe('多層級備援與零白屏防護 (Multi-Tier Fallback Resilience)', () => {
    it('當網路徹底中斷或 fetch 拋出例外時，主要權值股 (2330) 應回傳健全備援數據', async () => {
    ...
    it('伺服器回傳 HTTP 500 時，應平滑降級至備援', async () => {
  ...
  describe('快取機制與強制重新整理 (Cache Hit & Force Refresh Behavior)', () => {
    ...
    it('當設定 forceRefresh = true 時，應強制發起網路重取並更新快取', async () => {
      const mockFetch = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
        ok: true,
        status: 200,
        json: async () => ({
