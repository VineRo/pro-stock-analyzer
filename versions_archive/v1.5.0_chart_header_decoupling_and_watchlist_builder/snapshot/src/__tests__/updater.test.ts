...
describe('軟體自動更新核心安全演算法測試 (Updater Security & SemVer)', () => {
  ...
  describe('發布說明文字清洗測試 (Clean Release Notes HTML Tags)', () => {
    it('應能將帶有 <p> 與 <br> 標籤的日誌過濾為乾淨純文字', () => {
      const dirty = '<p>Release v1.4.0: Interactive Onboarding, 10-Year Candlestick Data</p><p>TWSE Registry Sync, and HUD Pinning</p>';
      const cleaned = cleanReleaseNotes(dirty);
      expect(cleaned).not.toContain('<p>');
      expect(cleaned).not.toContain('</p>');
      expect(cleaned).toContain('Release v1.4.0: Interactive Onboarding, 10-Year Candlestick Data');
      expect(cleaned).toContain('TWSE Registry Sync, and HUD Pinning');
    });
