...
export const ChartContainer: React.FC<ChartContainerProps> = ({
  ...
  useEffect(() => {
    ...
    // 增加 K 線主繪圖區上下安全緩衝，保證即使垂直縮放也絕不觸頂
  ...
  const renderedMainIndicators = useMemo(() => {
    ...
    if (mainIndicators.length > 0) {
      return (
        <div className="flex items-center gap-1.5 font-mono text-[11px] text-slate-400 border-l border-[#2e323e] pl-2.5 shrink-0">
          <span className="text-pro-muted">指標:</span>
          {mainIndicators.map((ind, idx) => (
            <span key={ind} style={{ color: maColors[idx % maColors.length] }} className="font-semibold">{ind}</span>
          ))}
        </div>
      );
    }
