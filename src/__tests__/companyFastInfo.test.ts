...
describe('公司資訊最速報與重大行事曆全套自動化測試 (Company Fast Info & MOPS)', () => {
  ...
  // ==========================================================================
  // 11. 當月重要消息判定與嚴格個股新聞關聯度過濾測試
  // ==========================================================================
  describe('當月重要消息判定與嚴格個股新聞關聯度過濾 (Current Month Announcements & Strict Stock News Filter)', () => {
    it('應精確判定是否為「當月」之公告日期', () => {
      const baseDate = new Date('2026-09-06T00:00:00Z');
      expect(isCurrentMonthDate('2026/09/05', baseDate)).toBe(true);
      expect(isCurrentMonthDate('2026-09-01', baseDate)).toBe(true);
      expect(isCurrentMonthDate('1150905', baseDate)).toBe(true); // 民國 115 年 9 月
      expect(isCurrentMonthDate('2026/08/30', baseDate)).toBe(false);
      expect(isCurrentMonthDate('2026/10/01', baseDate)).toBe(false);
    });

    it('isCurrentMonthOrRecent 應將當月及近 31 日內重大訊息判定為有效關注消息', () => {
      const baseDate = new Date('2026-09-06T00:00:00Z');
      expect(isCurrentMonthOrRecent('2026/09/02', baseDate)).toBe(true);
      expect(isCurrentMonthOrRecent('2026/08/18', baseDate)).toBe(true); // 19 天前，在 31 天內
      expect(isCurrentMonthOrRecent('2026/07/01', baseDate)).toBe(false); // 67 天前，超出 31 天
    });

    it('extractStockKeywords 應正確提取個股之關鍵字 (包含代號、純中文名、英文簡稱與別名，並剔除泛用市場詞)', () => {
      const tsmcKeywords = extractStockKeywords('2330', '台積電 (TSMC)');
      expect(tsmcKeywords).toContain('2330');
      expect(tsmcKeywords).toContain('台積電');
      expect(tsmcKeywords).toContain('tsmc');
      // 不應包含廣義產業詞
      expect(tsmcKeywords).not.toContain('半導體');
      expect(tsmcKeywords).not.toContain('大盤指數');
    });

    it('isNewsRelevantToStock 應嚴格篩選指名該個股之新聞，並過濾排除無關大盤與總經新聞', () => {
      // 案例 1: 明確指名台積電 (應通過)
      const news1 = isNewsRelevantToStock(
        '輝達次世代架構加速拉貨，台積電 3 奈米與 CoWoS 產能全線滿載至 2027',
        '工商時報 • 2 小時前 • AI 加速器需求爆發',
        '2330',
        '台積電'
      );
      expect(news1.isRelevant).toBe(true);
      expect(news1.matchedKeyword).toBe('台積電');

      // 案例 2: 僅含代號 2330 (應通過)
      const news2 = isNewsRelevantToStock(
        '外資回補 2330 逾萬張，推升加權指數大漲百點',
        '經濟日報',
        '2330',
        '台積電'
      );
      expect(news2.isRelevant).toBe(true);

      // 案例 3: 英文 TSMC (應通過)
      const news3 = isNewsRelevantToStock(
        'Apple and TSMC collaborate on next-generation 2nm architecture',
        'Reuters tech report',
        '2330',
        '台積電 (TSMC)'
      );
      expect(news3.isRelevant).toBe(true);

      // 案例 4: 泛市場/大盤宏觀新聞 (不提及台積電，應嚴格排除！)
      const newsMacro = isNewsRelevantToStock(
        '美國聯準會釋放降息訊號，全球股市與美債殖利率同步震盪',
        '華爾街日報 • 總體經濟與通膨數據最新解析',
        '2330',
        '台積電'
      );
      expect(newsMacro.isRelevant).toBe(false);

      // 案例 5: 報導其他無關公司 (如聯發科新聞，不應出現在台積電頁面)
      const newsOther = isNewsRelevantToStock(
        '聯發科發表最新天璣 9400 旗艦晶片，力拚全球市占率第一',
        '鉅亨網報導',
        '2330',
        '台積電'
      );
      expect(newsOther.isRelevant).toBe(false);

      // 案例 6: 鴻海個股新聞檢測
      const newsHonHai = isNewsRelevantToStock(
        '鴻海 GB200 AI 伺服器第四季放量出貨，毛利率顯著提升',
        '經濟日報報導',
        '2317',
        '鴻海'
      );
      expect(newsHonHai.isRelevant).toBe(true);
      expect(newsHonHai.matchedKeyword).toBe('鴻海');
    });

    it('應能精準識別台灣本土權威科技媒體 (科技新報、DIGITIMES、數位時代、鉅亨科技)', () => {
      // 科技新報 (TechNews)
      expect(isTaiwanTechMedia('科技新報', '台積電 2 奈米試產')).toBe(true);
      expect(isTaiwanTechMedia('TechNews', '先進封裝擴產')).toBe(true);

      // 電子時報 (DIGITIMES)
      expect(isTaiwanTechMedia('電子時報 (DIGITIMES)', 'CoWoS 產能吃緊')).toBe(true);
      expect(isTaiwanTechMedia('DIGITIMES', 'AI 晶片供應鏈')).toBe(true);

      // 數位時代 (Business Next)
      expect(isTaiwanTechMedia('數位時代', '生成式 AI 產業浪潮')).toBe(true);
      expect(isTaiwanTechMedia('Business Next', '黃仁勳演講')).toBe(true);

      // 鉅亨科技
      expect(isTaiwanTechMedia('鉅亨科技', '半導體大廠營收創高')).toBe(true);
      expect(isTaiwanTechMedia('鉅亨網', '台積電外資目標價')).toBe(true);

      // 財經媒體但報導半導體核心科技
      expect(isTaiwanTechMedia('經濟日報', '台積電晶圓代工與先進製程進度')).toBe(true);
      expect(isTaiwanTechMedia('工商時報', 'AI伺服器水冷散熱模組訂單')).toBe(true);

      // 宏觀無關新聞不應被識別為台灣科技報導
      expect(isTaiwanTechMedia('CNN', 'US election results')).toBe(false);
      expect(isTaiwanTechMedia('BBC', 'Global oil prices fluctuation')).toBe(false);
    });

    it('種子資料庫中台積電與鴻海新聞應正確標記 isTaiwanTech 且來源涵蓋本土科技媒體', () => {
      const tsmcSeed = getBenchmarkSeed('2330');
      expect(tsmcSeed).not.toBeNull();
      expect(tsmcSeed!.news.length).toBeGreaterThan(0);

      const tsmcSources = tsmcSeed!.news.map((n) => n.source);
      expect(tsmcSources.some((s) => s.includes('科技新報'))).toBe(true);
      expect(tsmcSources.some((s) => s.includes('DIGITIMES') || s.includes('電子時報'))).toBe(true);
      expect(tsmcSources.some((s) => s.includes('數位時代'))).toBe(true);
      expect(tsmcSeed!.news.every((n) => n.isTaiwanTech === true)).toBe(true);

      const foxconnSeed = getBenchmarkSeed('2317');
      expect(foxconnSeed).not.toBeNull();
      expect(foxconnSeed!.news.length).toBeGreaterThan(0);
      expect(foxconnSeed!.news.some((n) => n.source.includes('DIGITIMES'))).toBe(true);
      expect(foxconnSeed!.news.some((n) => n.source.includes('科技新報'))).toBe(true);
      expect(foxconnSeed!.news.every((n) => n.isTaiwanTech === true)).toBe(true);
    });
  });
});
