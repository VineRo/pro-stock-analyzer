...
export const App: React.FC = () => {
  // 1. 股票標的與市場分類分頁狀態 (股票幣圈 / 大盤指數)
  const [currentCategory, setCurrentCategory] = useState<MarketCategory>('stocks_crypto');
  ...
  const handleCategoryChange = useCallback((category: MarketCategory) => {
    ...
    if (category === 'indices') {
      ...
      setActiveTab('chart');
      // 若當前標的是大盤指數，切換為股票標的
      if (selectedSymbol.isIndex) {
        const defaultStock = POPULAR_SYMBOLS.find((s) => s.symbol === '2330.TW') || POPULAR_SYMBOLS[0];
        setSelectedSymbol(defaultStock);
      }
    }
  }, [selectedSymbol]);
